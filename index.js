...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ aguarde o processo filho da puta⌛',
			...
			switch(command) {
				...
				case 'hilih':
					...
					break
				case 'yt2mp3':
					if (args.length < 1) return reply('Onde está o url filho da puta?')
				...
				case 'ytsearch':
					...
					break
				case 'tiktok':
					if (args.length < 1) return reply('Cadê o url, fdp?')
				...
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ erro ao responder sticker ❌')
					reply(mess.wait)
				...
				case 'bem vindo':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('Bem vindo, seja ativo ou tome ban!❤️😏')
					if (Number(args[0]) === 1) {
